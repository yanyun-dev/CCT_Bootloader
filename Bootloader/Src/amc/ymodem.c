...
static int32_t Receive_Packet (uint8_t *data, int32_t *length, uint32_t timeout)
{
  ...
  uint8_t c;
  *length = 0;
  if (MainComm_GetChar(&c, timeout) == HAL_TIMEOUT)
  {
    return -1;
  }
  switch (c)
  {
    ...
    case EOT:
      return 0;
    case CA:
      if ((MainComm_GetChar(&c, timeout) == 0) && (c == CA))
      {
        *length = -1;
        return 0;
      }
  ...
  *data = c;
  ...
  {
    if (MainComm_GetChar(data + i, timeout) != 0)
    {
      return -1;
    }
  }
...
{
  uint8_t packet_data[PACKET_1K_SIZE + PACKET_OVERHEAD], file_size[FILE_SIZE_LENGTH], *file_ptr, *buf_ptr, eot_count = 0;
                ...
                if (packets_received == 0)
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)
                  {
                    ...
                    /* Erase the FLASH pages */										
										...
										FLASH_EraseInitStructure.PageAddress = FlashDestination;
										FLASH_EraseInitStructure.NbPages = NbrOfPage;
